...
export default function PublicProfile() {
  ...
  const {
    ...
    queryFn: async () => {
      /* 0 means start page, size 100 to get a bunch without complex pagination for now */ const {
        data,
      } = await axiosPublic.get(
        `/accepted-blogs?ownerId=${id}&page=0&size=100&search=${search}&sort=${sort}`,
      );
      return data;
    },
  });
    ...
    <>
      ...
      <main className="mx-auto my-12 min-h-screen w-11/12 max-w-screen-xl">
        ...
        <div className="mb-12 flex flex-col items-center justify-center rounded-2xl bg-gray-50 py-12 text-center dark:bg-gray-900/50">
          {" "}
          <img
            className="mb-4 h-32 w-32 rounded-full border-4 border-white object-cover dark:border-gray-800"
        ...
        <div>
          {" "}
          <div className="mb-6 flex flex-col items-center justify-between gap-4 border-b border-gray-200 pb-4 dark:border-gray-800 sm:flex-row">
            {" "}
          ...
          {blogs.length === 0 ? (
            <div className="rounded-xl border border-dashed border-gray-200 py-16 text-center dark:border-gray-800">
              ...
              <p className="mt-2 text-gray-600 dark:text-gray-400">
                Check back later for new content from {author.name}.
              </p>{" "}
            </div>
          ) : (
